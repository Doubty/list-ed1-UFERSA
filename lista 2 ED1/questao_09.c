...
tDado remover(tElemento * pInicio){
	...
	tElemento * p = buscar(pInicio, menorID(pInicio));
	tDado retorno; 
	retorno.id = -1;
...
int main(){
    ...
    menor = menorID(pInicio);
	
	printf("\n Menor iD: %d \n", menor );
	
			strcpy(entrada.nome, "gonzaga"); 
	entrada.valor = 30;
